...
const MODULE = path.join(__dirname, 'ret_step4_module.js');
const GUARD  = 'RET Step 4 module loaded';

const appSrc = fs.readFileSync(APP_JS, 'utf8');
const modSrc = fs.readFileSync(MODULE, 'utf8');

if (appSrc.includes(GUARD)) {
  console.log('✅  Guard hit — RET Step 4 already spliced. Skipping.');
  process.exit(0);
}

fs.writeFileSync(APP_JS, appSrc + '\n\n' + modSrc + '\n');
const newSize = fs.statSync(APP_JS).size;
console.log('✅  RET Step 4 spliced successfully.');
